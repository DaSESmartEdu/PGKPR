...
MID_WEIGHT = 0.5  # Reconstruction Loss
CON_WEIGHT = 0.01  # contrastive Loss
S2S_WEIGHT = 1.0   # s2s Loss

task_name = 'quora'
log_path = './grade_runs'
# base_path = 'data/quora/stage_two'
vocab_path = 'data/quora/quora.8000'

train_path = 'data/quora/stage_two_xy/train.csv'
train_path_xx = 'data/quora/stage_two_xx/train.csv'

val_path = 'data/quora/stage_two_xx/train.csv'
val_path_xy = 'data/quora/stage_two_xy/test.csv'
...
class SPLoss:
    def __init__(self, vocab_size, mid_weight=0.5, con_loss_weight=0.5, con_temperature=0.1, opt=None, if_con=True,
                 if_mid=True):
...
# model
model = PGKPR(d_model=D_MODEL, vocab=vocab_size, mid_loss=True, double_pe=DOUBLE_POSITION,
              shared_emb=True)
...
# model_opt = torch.optim.AdamW(model.parameters(), lr=1e-3, betas=(0.9, 0.98))
model = model.cuda()
...
for epoch in range(EPOCHS):
    ...
    for phase in ['train', 'val', 'train_xx', 'val']:
        ...
        epoch_s2s = total_con_loss / len(dataloader[phase])
        ...
        if 'val' in phase and (epoch_loss < best_loss or epoch % 10 == 0 or epoch < 20) and epoch > 5:
            print("save model")
